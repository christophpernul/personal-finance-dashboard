...
def parse_etf_master_data(html: str):
    ...
    assert isinstance(
        soup_base, type(BeautifulSoup())
    ), "EXTRACT: Input is no BeautifulSoup object!"
    metadata = {}
    ### Get name of stock
    stock_name = (
    ...
    for box in infoboxes:
        try:
            value = (
                box.find_all("div", {"class": "val"})[0]
                .text.replace(" ", "")
                .replace("\n", "")
            )
            label = (
                box.find_all("div", {"class": "vallabel"})[0]
                .text.replace(" ", "")
                .replace("\n", "")
            )
        except IndexError:
...
def parse_etf_prices(html: str):
    ...
    assert isinstance(
        soup_base, type(BeautifulSoup())
    ), "Input is no BeautifulSoup object!"
    price_dict = {}

    price_obj = (
        soup_base.find_all("div", {"class": "infobox"})[0]
        .find_all("div", {"class": "val"})[0]
        .find_all("span")
    )
